...
function setupAudioEngine() {
    ...
    // ── Multi-Room Websocket Bridging Client ──
    let wsConnection = null;
    let wsReconnectTimeout = null;

    function initWebsocketBridge(wsUri) {
        if (wsConnection) {
            wsConnection.close();
        }

        console.log("[AgentIRC] Bridging WS connecting to:", wsUri);
        wsConnection = new WebSocket(wsUri);

        wsConnection.onopen = function() {
            console.log("[AgentIRC] Bridging WS DATALINK ESTABLISHED");
            renderWsBridgeMessage({
                "kind": "system",
                "content": `DATALINK ESTABLISHED -> ${wsUri}`
            });
        };
        ...
        wsConnection.onmessage = function(event) {
            try {
                ...
                // Handle JSON-RPC MCP payload
                if (data.jsonrpc === "2.0" && data.method === "agentirc/payload") {
                    const payload = data.params.payload || {};
                    renderWsBridgeMessage(payload);
                } else {
                    console.warn("[AgentIRC] Unknown WS format received:", data);
                }
            } catch (err) {
                console.error("[AgentIRC] Failed to parse WS message", err);
            }
        };

        wsConnection.onclose = function() {
            console.warn("[AgentIRC] Bridging WS DATALINK LOST");
            renderWsBridgeMessage({
                "kind": "system",
                "content": "DATALINK LOST - RECONNECTING..."
            });
            clearTimeout(wsReconnectTimeout);
            wsReconnectTimeout = setTimeout(() => {
                initWebsocketBridge(wsUri);
            }, 5000);
        };
    }

    function renderWsBridgeMessage(payload) {
        // Look for the Chainlit message container, if possible
        const messagesContainer = document.querySelector(".cl-message-list") || document.body;

        const wrapper = document.createElement("div");
        wrapper.className = "step-tool irc-ws-bridge-msg";
        wrapper.style.backgroundColor = "#000080";
        wrapper.style.color = "#ffffcc";
        wrapper.style.border = "1px dashed #ffffcc";

        let headerText = "[WS \u2192 #local]";
        if (payload.source_room) {
            headerText = `[WS \u2192 #${payload.target_room || 'local'} from #${payload.source_room}]`;
        }

        wrapper.innerHTML = `<strong>${headerText}</strong><br />${payload.content || JSON.stringify(payload)}`;

        // Append into standard flow
        messagesContainer.appendChild(wrapper);
        // Scroll to bottom
        window.scrollTo(0, document.body.scrollHeight);
    }

    // Auto-initialize if body has data-ws-bridge
    window.addEventListener("load", () => {
        const bodyTag = document.body;
        const uri = bodyTag.getAttribute("data-ws-bridge");
        if (uri) {
            initWebsocketBridge(uri);
        }
    });
