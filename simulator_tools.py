...
def _resolve_sandbox_path(filename: str) -> str:
    """Ensure the filename is safely restricted to the sandbox directory."""
    safe_name = os.path.basename(filename)
    return os.path.join(SANDBOX_DIR, safe_name)

def sandbox_read_file(filename: str) -> str:
    """Read a text file from the local simulation sandbox environment. Good for agent notes."""
    safe_path = _resolve_sandbox_path(filename)
    if not os.path.exists(safe_path):
        return f"Error: Sandbox file '{filename}' does not exist."
    try:
...
def sandbox_write_file(filename: str, content: str) -> str:
    """Write text content to a file in the local simulation sandbox environment."""
    safe_path = _resolve_sandbox_path(filename)
    try:
        with open(safe_path, 'w', encoding='utf-8') as f:
            f.write(content)
        return f"Successfully wrote to sandbox file: {filename}"
    except Exception as e:
