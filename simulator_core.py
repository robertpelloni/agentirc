...
- `/add-model <name> <provider> <model_id> ["persona override"]` - Add a new model dynamically (e.g. use provider "ollama" and model_id "llama3").
...
def build_dashboard_text(
    ...
    return (
        ...
        "|---|---|---|\n"
        f"| Active room | `{current_room_name}` | <span title='The current operational environment'>Current active simulation room</span> |\n"
        f"| Active topic | {active_room.get('topic', DEFAULT_TOPIC)} | <span title='The prompt directive guiding agent behavior'>Current active conversation topic</span> |\n"
        f"| Active mode | `{active_room.get('mode', DEFAULT_MODE)}` | <span title='Whether the agents are broadcasting sequentially or discussing dynamically'>Current interaction mode</span> |\n"
        f"| Total rooms | `{len(rooms)}` | <span title='Number of segmented environments available'>Total isolated environments</span> |\n"
        f"| Retained transcript entries | `{total_entries}` | <span title='Total number of messages kept in memory across all rooms'>Total history size</span> |\n"
        f"| Aggregate prompts sent | `{total_prompts}` | <span title='Total interactions initiated by the user'>User prompts sent</span> |\n"
        f"| Aggregate bridge events | `{total_bridges}` | <span title='Total cross-room transmissions'>Messages bridged</span> |\n"
        f"| External exports | `{sum(room_state.get('config', {}).get('telemetry', {}).get('external_exports', 0) for room_state in rooms.values())}` | <span title='Total outbox payloads exported'>Exported items</span> |\n"
        f"| External imports | `{sum(room_state.get('config', {}).get('telemetry', {}).get('external_imports', 0) for room_state in rooms.values())}` | <span title='Total inbox payloads imported'>Imported items</span> |\n"
        f"| Saved lineups | `{len(persistent_state.get('saved_lineups', {}))}` | <span title='Pre-configured sets of enabled agents'>Saved agent combinations</span> |\n"
        f"| Saved jobs | `{len(persistent_state.get('saved_jobs', {}))}` | <span title='Pre-configured autonomous tasks'>Saved automation jobs</span> |\n"
        f"| Aggregate estimated cost | `{format_usd(total_estimated_cost)}` | <span title='Total API cost based on token usage across all runs'>Estimated LLM inference cost</span> |\n"
        "\n**Available Sub-Systems**\n"
        "- `/bridge-websocket <ws_uri>`: Spawn background listener for remote payload execution.\n"
        "- `/go <room>`: Enter MUD mode and roleplay as NPCs in interconnected physical rooms.\n"
        "- `pr_review_agents.py`: Headless system for fetching GitHub diffs and holding code-review discussions.\n"
        "- `discord_bridge.py`: Headless system for bi-directional bot mirroring.\n"
        "- `headless_irc.py`: Headless system for running IRC clients against standard servers.\n"
    )
